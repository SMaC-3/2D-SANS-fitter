...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  4 11:42:27 2019
...
#        self.beamStop = np.logical_or(bsq < self.qmin, bsq>0.15)
        ...
        #testing resolution
        self.expData = sasmodels.data.Data2D(x = self.expData_sort[:,0], dx = 0.1*abs(self.expData_sort[:,0]), y = self.expData_sort[:,1] , dy = 0.1*abs(self.expData_sort[:,1]), z = self.expData_sort[:,2], dz = self.expData_sort[:,3])
        self.expData.sample = sample
        self.expData.shear = shear

        return
    
    def interpData(self, dataSet):
...
#        zzq = griddata(dataSet[:,0:2], dataSet[:,2], (self.xxq,self.yyq), method = 'linear')
        zzq1 = griddata((dataSet.qx_data, dataSet.qy_data), dataSet.data , (self.xxq,self.yyq), method = 'linear')
...
#        self.beamStop = (bsq < self.qmin)
        ...
        self.simImport_sort = np.array(sorted(self.simImport_masked, key=lambda col: (col[1], col[0])))
        
        self.simImport = sasmodels.data.Data2D(x = self.simImport_sort[:,0] , y = self.simImport_sort[:,1] , z = self.simImport_sort[:,2])
...
# =============================================================================
        ...
        self.simData = sasmodels.data.empty_data2D(self.qx_unique, self.qy_unique,resolution=0.1) #Needed for calculator? using q from class definition as want all calcs to be based on the same grid
        ...
        #print(arrayDif)
        ranI = 1000
        self.arrayDif_z = np.insert(arrayDif, 2, ranI, axis=1) #(x,y) pairs in simulation not in experiment with constant z
        #print(arrayDif_z)
        exp_data = self.expData_sort[:,0:3]
        #print(exp_data)
        self.expData_fill = np.vstack((exp_data,self.arrayDif_z))
        self.expData_fill_sort = np.array(sorted(self.expData_fill, key=lambda col: (col[1], col[0])))
        
        #print(exp_dataFill)
        exp_dataFillSas = sasmodels.data.Data2D(self.expData_fill_sort[:,0], self.expData_fill_sort[:,1], self.expData_fill_sort[:,2])
        
        self.simData.mask = np.logical_or(exp_dataFillSas.data == ranI, exp_dataFillSas.q_data < self.qmin)
...
#        fcylhs = sasmodels.core.load_model_info('flexible_cylinder@hardsphere') 
        ...
        model = sasmodels.core.build_model(cylhmsa, platform = 'dll') #Build using c version instead of python. Avoids pyopencl    
...
# =============================================================================
        ...
        self.simMask = sasmodels.data.Data2D(x=qx_mask, y=qy_mask, z=scat, dz = np.array(list(itertools.repeat(10, len(qx_mask)))))
...
#        q, binave, errave = ansect.sector(self., centre, width, '0', describer = None)
        ...
        expHorizq, expHorizI, expHorizerr = ansect.sector(exp, np.pi/2, np.pi/20, '0', describer = None)
        simHorizq, simHorizI, simHorizerr = ansect.sector(sim, np.pi/2, np.pi/20, '0', describer = None)
        ...
        if pltErr == 1:
            ...
            ax3.plot(expq,expI, marker='o',markersize = 2,markerfacecolor = [0,0,0], linestyle = '', label = 'dataRad')
            ax3.plot(simq,simI, marker='o',markersize = 2,markerfacecolor = [0,0,0], linestyle = '', label = 'simRad')
            ax3.set_xscale('log')
            ax3.set_yscale('log')
            ax3.legend()
            ax3.minorticks_off()
...
#            plt.xticks(fontsize=8)
            ...
            ax3 = fig.add_subplot(1, 3, 3)
            ax3.plot(expq,expI, marker='',markersize = 2,markerfacecolor = [0,0,0], linestyle = '-', label = 'dataRad')
            ax3.plot(simq,simI, marker='',markersize = 2,markerfacecolor = [0,0,0], linestyle = '-', label = 'simRad')
            ax3.set_xscale('log')
            ax3.set_yscale('log')
            ax3.legend()
            ax3.minorticks_off()
...
# =============================================================================
    ...
    while i < numParas:
        ...
        (numList, step) = np.linspace( listInfo[i][1][0] , listInfo[i][1][1] , listInfo[i][1][2], retstep = 'true')
        
#        lists.append( numList[1:-1] )
        lists.append(numList)
       
        paraInfo.update({listInfo[i][0][0] + '_npts' : listInfo[i][1][2], listInfo[i][0][0] + '_steps' : step })
...
# =============================================================================
    ...
    for para, vals in parsUpdate[minResI].items():
        listInfo2.append( [[para], [vals - paraInfo[para + '_steps'], vals + paraInfo[para + '_steps'], paraInfo[para + '_npts']]] )
...
def plot(sim, zzq, bounds):
    ...
    plt.pcolormesh(sim.xxq, sim.yyq, zzq, cmap='jet', vmin=bounds[0], vmax=bounds[1], norm=matplotlib.colors.LogNorm())
    plt.title('')
...
#############################################################################
def save(sans, describer, minParams, minPars, stats, location):

#
# =============================================================================
    
    #location = '../2D_simFits/ReducedChi2_fits/20wt_CAPB_SLES_2wt_NaCl/'
    describer = '_' + describer
    
    name = sans.setName[15:-4]
    post1 = '_sim'
    type1 = '.dat'
    
    saveName1 = name + post1 + describer
    
    post2 = '_simInfo'
    type2 = '.txt'
    
    saveName2 = name + post2 + describer
...
#    print(output)
    ...
    if path.exists(location + saveName2 + type2):
        ...
        with open(location + saveName2 + type2, 'w') as file:
            for lines in output:
                file.write(lines)
                file.write("\n")
...
#        versionNum = input("Input a version number: ")
        ...
        write_3_column(location + saveName1 + type1, sans) 
        
    print('file was saved with filename: ' + saveName1)
    return
...
# =============================================================================
    ...
    with open(filename, 'wt') as fh:
        fh.write("Qx  Qy  I\n")
        for x, y, z in zip(sans.simMask.qx_data, sans.simMask.qy_data, sans.simMask.data):
            fh.write("%g  %g  %g\n" % (x, y, z))
