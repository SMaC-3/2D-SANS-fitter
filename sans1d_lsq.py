...
indexSelected = '2'
p_list = ['background', 'length', 'scale', 'radius_effective']
p_guess = [0.28, 250, 0.91, 65]
...
#
    ...
    if save == '1' or save == '':
        ...
        sans1d.write_3_column(location + sans.description + '_sim' +
                              fileType, sans.expData.x, optimSim)
        sans1d.saveStats(location + sans.description + '_stats' + fileType2, minParams=sans.pars,
                         minPars=dict(zip(p_list, optim.x)), chi2='chi2_r: ' + str(np.nansum(optim.fun)), res='')
